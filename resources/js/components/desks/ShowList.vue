<template>
   ...
   <div v-for="card in cards" :key="card.id" class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
         ...
         <router-link :to="{name: 'showCard', params: {cardid: card.id, cardname: card.name, list_id: listId}}" href="#" class="mb-3 inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            Редактировать
